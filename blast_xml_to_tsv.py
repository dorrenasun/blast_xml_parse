...
# Modified by Rui Sun (2020) dorrenasun@gmail.com
...
def parse_ncbi_xml_to_tsv(infile, outfile, delim = '\t'):
	...
	r_query_num = re.compile("<Iteration_iter-num>")
	...
	for line in open(infile).readlines():
		if r_query_num.search(line) is not None:
			query_num = line.split("<Iteration_iter-num>")[1].split("<")[0]
		elif r_query_def.search(line) is not None:
			query_def = line.split("<Iteration_query-def>")[1].split("<")[0]
			query_acc = query_def.split()[0]
			...
			percent_pos = "%5.2f" %  (((float(hsp_positive)/float(hsp_align_len))*100))
			...
			#################################################################################################################################
			#header_list = ['query_num', 'query_acc', 'query_def', 'query_len', 'hit_num', 'hit_id', 'hit_def', 'hit_acc', 'hit_len', 'bit_score', 'score', 'evalue', 'q_start', 'q_end', 's_start', 's_end', 'n_iden', 'n_pos', 'mismatches', 'gaps', 'align_len', 'per_iden', 'per_pos']

			var_list = [query_num, query_acc, query_def, query_len, \
						hit_num, hit_id, hit_def, hit_accession, hit_len, \
						hsp_bit_score, hsp_score, hsp_evalue, \
						hsp_query_from, hsp_query_to, hsp_hit_from, hsp_hit_to, \
						hsp_identity, hsp_positive, mismatch, hsp_gaps, hsp_align_len, percent_ident, percent_pos]
...
def main():
	...
	infile, outfile = input_checker(parser.parse_args())
	...
	parse_ncbi_xml_to_tsv(infile, outfile)
